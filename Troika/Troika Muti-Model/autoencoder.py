...
def autoencoder(input_shape, weight_decay=1e-4, dropout_rate=0.2):
	'''
	...
	encoder = Conv1D(64, 4, padding='same',
						kernel_initializer='glorot_uniform',
							kernel_regularizer=l2(weight_decay))(encoder)
	encoder = Dropout(dropout_rate)(encoder)
	encoder = MaxPooling1D(pool_size=5)(encoder)
	encoder = Activation('relu')(encoder)
	encoder = Conv1D(128, 4, padding='same',
						kernel_initializer='glorot_uniform',
							kernel_regularizer=l2(weight_decay))(encoder)
	encoder = Dropout(dropout_rate)(encoder)
	encoder = MaxPooling1D(pool_size=2)(encoder)
	...
	decoder = Conv1D(128, 4, padding='same',
						kernel_initializer='glorot_uniform',
							kernel_regularizer=l2(weight_decay))(decoder)
	decoder = Dropout(dropout_rate)(decoder)
	decoder = UpSampling1D(5)(decoder)    
	...
	decoder = UpSampling1D(2)(decoder)
	decoder = Activation('relu')(decoder)
	decoded = Conv1D(5, 4, padding='same',
						kernel_initializer='glorot_uniform',
							kernel_regularizer=l2(weight_decay))(decoder)
